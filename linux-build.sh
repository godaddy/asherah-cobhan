...
LD_RUN_PATH=\$ORIGIN CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build -v -buildmode=c-shared -o output/libasherah-arm64.so

LD_RUN_PATH=\$ORIGIN CGO_ENABLED=1 GOOS=linux GOARCH=arm64 go build -v -buildmode=c-shared -tags=debugoutput -o output/libasherah-debug-arm64.so
